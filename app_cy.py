...
# ── Page config ───────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="Cyprus Political Ads Monitor",
    page_icon="🗳️",
    layout="wide",
)
...
col6.metric(
    "Removed by Meta",
    f"{removed_n:,}",
    help=f"Checked: {checked_n:,} / {len(df):,}" if checked_n else "Run check_removed_ads_cy.py",
)
...
with tab_overview:
    ...
    with c2:
        st.subheader("Ads by District")
        dist_counts = df['district'].value_counts().reset_index()
        dist_counts.columns = ['District', 'Ads']
    ...
    cand_summary = (
        ...
        .reset_index()
        .rename(columns={'candidate': 'Candidate', 'party': 'Party', 'district': 'District'})
...
with tab_leaderboard:
    ...
    if spend.empty:
        ...
        st.dataframe(
            spend_display[['Candidate', 'Party', 'District', 'Spend Range (EUR)', 'Ads', 'Impressions (max)']],
...
with tab_pages:
    ...
    pages_summary = (
        df.groupby(['page_name', 'page_id', 'candidate', 'party', 'district'])
        .agg(
            Ads      =('ad_archive_id',  'count'),
            Active   =('ad_stop_date',   lambda x: x.isna().sum()),
            Impr_Max =('impressions_max','sum'),
    ...
    pages_summary['Ad Library'] = pages_summary['Page ID'].apply(
        lambda pid: (
            f"https://www.facebook.com/ads/library/?active_status=all"
            f"&ad_type=all&country=CY&media_type=all&view_all_page_id={pid}"
        )
    )
    ...
    st.dataframe(
        pages_summary[['Candidate', 'Party', 'District', 'Page Name', 'Own Page?',
...
with tab_active:
    ...
    active_summary = (
        df_active.groupby(['candidate', 'party', 'district'])
        .agg(
            Ads      =('ad_archive_id',  'count'),
            Impr_Max =('impressions_max','sum'),
            Spend_Max=('spend_max',      'sum'),
    ...
    st.divider()
    df_act_disp = df_active[['candidate','party','district','page_name','ad_start_date','impressions_max','spend_max','ad_url']].rename(columns={
        'candidate':'Candidate','party':'Party','district':'District','page_name':'Page',
...
with tab_inactive:
    ...
    inactive_summary = (
        df_inactive.groupby(['candidate', 'party', 'district'])
        .agg(
            Ads      =('ad_archive_id',  'count'),
            Impr_Max =('impressions_max','sum'),
            Spend_Max=('spend_max',      'sum'),
    ...
    st.divider()
    df_inact_disp = df_inactive[['candidate','party','district','page_name','ad_start_date','ad_stop_date','impressions_max','spend_max','ad_url']].rename(columns={
        'candidate':'Candidate','party':'Party','district':'District','page_name':'Page',
...
with tab_removed:
    ...
    if df_removed.empty:
        st.info("No ads marked as removed yet. Run check_removed_ads_cy.py to check.")
    else:
        removed_summary = (
            df_removed.groupby(['candidate', 'party', 'district'])
            .agg(
        ...
        st.divider()
        df_rem_disp = df_removed[['candidate','party','district','page_name','ad_start_date','ad_stop_date','impressions_max','spend_max','ad_url']].rename(columns={
            'candidate':'Candidate','party':'Party','district':'District','page_name':'Page',
            'ad_start_date':'Start','ad_stop_date':'Stop','impressions_max':'Impr Max','spend_max':'Spend Max','ad_url':'Ad'
        })
...
with tab_table:
    ...
    st.download_button(
        ...
        file_name="cyprus_political_ads_export.xlsx",
