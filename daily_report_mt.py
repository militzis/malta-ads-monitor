"""
daily_report_mt.py — Malta daily ad-monitoring report.
───────────────────────────────────────────────────────
Generates three reports each day:

  1. NEWLY REMOVED ADS
     Active election ads detected as removed in the last N hours.
  ...
  3. NEW ADS TODAY
     Ads first seen within the last N hours.

Output
  • Console summary (always)
  • Excel file  daily_report_mt_YYYY-MM-DD.xlsx  (use --no-excel to skip)

Usage
    python daily_report_mt.py                  # last 24h, writes Excel
    python daily_report_mt.py --hours 48       # look back 48h
    python daily_report_mt.py --no-excel       # console only
    python daily_report_mt.py --out mydir      # custom output folder
"""
...
def get_new_ads(conn, blocklist: set, hours: int) -> list[dict]:
    ...
    Falls back to ad_start_date (not checked_at) so re-fetched old ads
    don't appear as new.
    """
    cutoff = (datetime.now(timezone.utc) - timedelta(hours=hours)).isoformat()
...
def get_readvertisers(conn, blocklist: set, hours: int) -> list[dict]:
    """
    Pages that have at least one removed election ad AND at least one
    active ad first seen within the last `hours` hours whose start date
    is after their first detected removal.
    """
    cutoff = (datetime.now(timezone.utc) - timedelta(hours=hours)).isoformat()
...
def write_excel(removed_rows: list[dict], readv_rows: list[dict],
    ...
    fname = os.path.join(out_dir, f"daily_report_mt_{today}.xlsx")
...
def main():
    parser = argparse.ArgumentParser(description="Malta daily ad monitoring report.")
    ...
    print(f"\n{'═'*70}")
    print(f"  Malta Daily Ad Report — {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print(f"{'═'*70}")
